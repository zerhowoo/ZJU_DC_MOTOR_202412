# modified by Wang,Zihao at 2024.12.11 in Institute of Advanced Machines Zhejiang University
...
# set max. velocity
MAX_VELOCITY_ARM = 200000         # 手臂电机最大速度
MAX_VELOCITY_SILDE = 400000       # 滑台电机最大速度
num = 3                           # 两个手臂电机，一个滑台电机
...
def load_trajectory_points(file_path):
    ...
    with open(file_path, 'r') as file:
        for line in file:
            ...
            if line:
                try:
                    position1, position2, position_maxon, start_time, duration = map(float, line.split(','))   # 增加从站
                    trajectory_points.append(([position1, position2, position_maxon], start_time, duration))   # 增加从站
                except ValueError:
                    raise ValueError(f"文件格式错误：{line} 需要是 'position1, position2, position_maxon, start_time, duration' 格式。") # 增加从站
...
class DCControlNode(Node):
    def __init__(self, trajectory_points):
        ...
        self.current_state = "WAIT"                   # 设定 初始状态为等待 
        self.current_position = [0.0, 0.0, 0.0]       # 记录 当前两个机械臂小电机的位置 和 一个滑台电机的位置
        self.target_position = [0.0, 0.0, 0.0]        # 记录 当前两个机械臂小电机的目标位置 和 一个滑台电机的位置
        self.previous_target_position = [None, None, None]  # 记录 上一个目标位置 
        self.cycle_completed = True                   # 记录 是否完成当前轨迹
        self.start_time = None                        # 记录 运动开始的时间

        # configuration of target trajectorypoint
        self.trajectory_points = trajectory_points
        self.validate_trajectory_points()             # 验证轨迹点的有效性
    ...
    def validate_trajectory_points(self):
        ...
        for i in range(1, len(self.trajectory_points)):
            ...
            arm_velocities = [
                ...
                for j in range(num-1)                                                                 # 增加从站 position1, position2
            ]
            slide_velocity = abs(current_position[num-1] - prev_position[num-1]) / current_duration    # 增加一个从站 position_maxon 在第三位

            if any(v > MAX_VELOCITY_ARM for v in arm_velocities):
                raise ValueError(
                    f"第 {i + 1} 行的速度 ({arm_velocities}) 超过最大速度 ({MAX_VELOCITY_ARM}). "
                    f"请调整 position 或 duration."
                )
            if slide_velocity > MAX_VELOCITY_SILDE:
                raise ValueError(
                    f"第 {i + 1} 行的速度 ({slide_velocity}) 超过最大速度 ({MAX_VELOCITY_SILDE}). "
                    f"请调整 position 或 duration."
                )
    ...
    def listener_callback(self, msg):
        """订阅回调函数，更新当前电机位置"""
        self.current_position = list(msg.position[:num])                    # 增加从站 # 两个关节电机, 一个滑台电机 = 3
        ...
        if self.current_state == "MOVING":
            ...
            if all(abs(self.current_position[i] - self.target_position[i]) < 4 for i in range(num)) and current_time >= target_time:  # 增加从站 range(3)
    ...
    def publish_trajectory(self):
        ...
        msg.joint_names = ['joint_0', 'joint_1', 'joint_maxon']  
